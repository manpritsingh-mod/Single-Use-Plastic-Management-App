...
import com.manprit.plastic_management.Adapter.RecycleHistoryAdapterRecycler
...
class HistoryFragmentRecycler : Fragment() {
    ...
    override fun onCreateView(
        ...
        recyclerLayout.adapter = RecycleHistoryAdapterRecycler(activity as Context, statusDetailDataArray)
                    ...
                    if (snapshot.exists()) {
                        for (dataSnapShot in snapshot.children) {
                            if (dataSnapShot.key != "test") {
                                val detailsData = dataSnapShot.getValue(RequestDetailsData::class.java)
                                if (!(detailsData!!.Status.toString()
                                        .equals("Pending") || detailsData!!.Status.toString()
                                        .equals("Accepted"))
                                )
                                    if(detailsData!!.acceptedByUid.toString()
                                            .equals(uid))
...
//                            RecycleHistoryAdapter(activity as Context, statusDetailDataArray)
                        ...
                        recyclerLayout?.adapter = activity?.let { RecycleHistoryAdapterRecycler(it, statusDetailDataArray) }
