...
import com.manprit.plastic_management.Adapter.RecycleStatusDestroyerAdapter
...
class StatusFragmentDestroyer() : Fragment(),OnButtonClicked {
...
//    }
    ...
    override fun onCreateView(
        ...
        val statusDetailDataArray = arrayListOf<RecyclerFormData>()
        ...
        recyclerLayout.adapter = RecycleStatusDestroyerAdapter(activity as Context, statusDetailDataArray,btnClick,uid!!)
            ...
            database.addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        recyclerLayout.layoutManager = LinearLayoutManager(activity)
//                        recyclerLayout.adapter = RecycleStatusDestroyerAdapter(
//                            activity as Context,
//                            statusDetailDataArray,
//                            btnClick
//                        )

                        statusDetailDataArray.sortWith(compareByDescending<RecyclerFormData> {
                            SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).parse(it.Date)
                        }.thenByDescending {
                            SimpleDateFormat("HH:mm", Locale.getDefault()).parse(it.Time)
                        })

                        recyclerLayout?.adapter = activity?.let { RecycleStatusDestroyerAdapter(it, statusDetailDataArray,btnClick,uid) }
